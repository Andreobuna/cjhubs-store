...
export function CategoryGrid({ categories }: { categories: { name: string; slug: string; _count?: { products: number }; products?: { name: string; images: { url: string }[] }[] }[] }) {
    ...
    <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2">
        {categories.map((cat) => {
          ...
          const latestProduct = cat.products?.[0];
          const imageUrl = latestProduct?.images[0]?.url;
          return (
            <Link
              key={cat.slug}
